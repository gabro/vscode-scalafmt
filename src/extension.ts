...
function retrieveConfig(): string | undefined {
  const configFilePath = vscode.workspace
    ...
    .configFilePath.replace("${workspaceRoot}", vscode.workspace.rootPath);
      ...
      new Promise<vscode.TextEdit[]>((resolve, reject) => {
        try {
          ...
          const formattedFile = format(
            ...
            scalafmtConfig,
            range
              ? [{ start: range.start.line, end: range.end.line + 1 }]
              : undefined
          );
          output.clear();
          resolve([
            vscode.TextEdit.replace(fullDocumentRange(document), formattedFile)
          ]);
        } catch (e) {
          console.error(e);
          output.clear();
          output.appendLine(
            "An error occurred while formatting this file with Scalafmt:\n"
          );
          output.append(e);
          output.appendLine(
...
function showOrHideScalafmtStatusBarItem(
...
function announceDeprecation(context: vscode.ExtensionContext) {
  const panel = vscode.window.createWebviewPanel(
    "vscode-scalafmt-deprecation",
    "vscode-scalafmt is deprecated",
    ViewColumn.Active
  );
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const statusBarItemDisplayer = vscode.window.onDidChangeActiveTextEditor(
    editor => {
      showOrHideScalafmtStatusBarItem(editor, statusBarItem);
    }
  );
  showOrHideScalafmtStatusBarItem(
    vscode.window.activeTextEditor,
    statusBarItem
  );

  const openScalafmtOutput = vscode.commands.registerCommand(
    "scalafmt.open-output",
    () => {
      scalafmtOutput.show();
    }
  );
